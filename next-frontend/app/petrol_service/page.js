        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            async (position) => {
              ...
              // Fetch petrol service data
              try {
                const response = await fetch(`${API_BASE_URL}/api/services/type/petrol/`);
                if (response.ok) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        >
              ...
              <h1 className="text-3xl font-bold text-gray-900 mb-2 heading-responsive">Petrol Delivery Request</h1>
              <p className="text-gray-600 text-responsive">Emergency fuel delivery to your location</p>
              {serviceData && (
                <motion.div 
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium text-green-800">Base Amount:</span>
                    <span className="font-medium">₹{totalAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium text-green-800">Service Charge:</span>
                    <span className="font-medium">₹{serviceCharge}</span>
                  </div>
                  <div className="border-t border-green-200 pt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-bold text-green-900">Total Amount:</span>
                      <span className="text-lg font-bold text-green-900">₹{(parseFloat(totalAmount) + parseFloat(serviceCharge)).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
